	...
	Minesweeper game
		Starting state
			...
			**RULES OF MINESWEEPER**
			...
			Map is updated so that non-bomb positions are given a number 'a' [0, 8], 
			where 'a' is the number of neighbors that have bombs
			Each position is either revealed or hidden, initially all states are hidden
			**BONUS: Don't initialize map until first action, so that it is guarenteed that the first action 
			         is not to click on a bomb (generate map after first action (first click))
